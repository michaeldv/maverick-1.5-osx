...
struct t_pawn_hash_record *lookup_pawn_hash(struct t_board *board, struct t_chess_eval *eval)
{
    ...
    for (color = WHITE; color <= BLACK; color++) {
		...
		//-- Add an endgame bonus for king next to passed pawn
		endgame += popcount(b & king_mask[board->king_square[color]]) * 10 * (1 - color * 2);
		...
		//-- Unstopable
		pawn_record->unstopable[color] = pawn_record->passed[color] & cannot_catch_pawn_mask[opponent][board->to_move][board->king_square[opponent]];
	}
	...
	//-- Central e4-d4 duo
	for (color = WHITE; color <= BLACK; color++){
		if ((board->pieces[color][PAWN] & central_duo[color]) == central_duo[color]){
			opponent = OPPONENT(color);
			b = board->pieces[opponent][PAWN] & central_duo_defence_mask[opponent];
			if ((b == central_duo_defence[opponent][0]) || (b == central_duo_defence[opponent][1]))
				middlegame += 7 * (1 - color * 2);
			else
				middlegame += 15 * (1 - color * 2);
		}
	}
	...
	//	t_bitboard moves = (((b << 8) >> (16 * color)) & ~(board->all_pieces));
	//	t_bitboard double_push = (moves & rank_mask[color][THIRD_RANK]);
	//	double_push = (((double_push << 8) >> (16 * color)) & ~(board->all_pieces));

	//	t_chess_value mobility = popcount(moves | double_push);

	//	moves = (((b & B8H1) << 7) >> (16 * color)) & board->occupied[opponent];
	//	mobility += popcount(moves);

	//	moves = (((b & A8G1) << 9) >> (16 * color)) & board->occupied[opponent];
	//	mobility += popcount(moves);

	//	middlegame += 2 * mobility * (1 - 2 * color);
	//	endgame += 2 * mobility * (1 - 2 * color);
	//}
	...
	//-- Pawn Chains
	evaluate_pawn_chains(board, pawn_record);
...
void evaluate_pawn_chains(struct t_board *board, t_pawn_hash_record *pawn_record){

	for (t_chess_color color = WHITE; color <= BLACK; color++)
	{
		t_bitboard b = board->pieces[color][PAWN];
		t_bitboard a1h8 = ((b & A8G1) << 9) & b & ((b & B8H1) >> 9);
		t_bitboard a8h1 = ((b & A8G1) >> 7) & b & ((b & B8H1) << 7);

		t_bitboard inward, outward;
		t_chess_square s;
		...
		if (color == WHITE){
			inward = (a1h8 & BITBOARD_QUEENSIDE) | (a8h1 & BITBOARD_KINGSIDE);
			outward = (a1h8 & BITBOARD_KINGSIDE) | (a8h1 & BITBOARD_QUEENSIDE);
		}
		else{
			inward = (a1h8 & BITBOARD_KINGSIDE) | (a8h1 & BITBOARD_QUEENSIDE);
			outward = (a1h8 & BITBOARD_QUEENSIDE) | (a8h1 & BITBOARD_KINGSIDE);
		}

		while (inward)
		{
			s = bitscan_reset(&inward);

			middlegame += inward_chain[color][s];
			endgame += inward_chain[color][s];
		}

		while (outward)
		{
			s = bitscan_reset(&outward);

			middlegame += 5 * (1 - color * 2);
			endgame += 5 * (1 - color * 2);
		}
...
void evaluate_king_pawn_endgame(struct t_board *board, struct t_pawn_hash_record *pawn_record)
{
    ...
    for (t_chess_color color = WHITE; color <= BLACK; color++) {
		...
		t_bitboard unstoppables = pawn_record->unstopable[color];
